...
module type Ship = sig
  ...
  type ship = {
    (* [resources] is (fuel, missiles, scraps) *)
  ...
  (* regen shield, called each tick *)
  val regen_shield : ship -> ship

  (* whether [i]th weapon is ready to fire 
   * if [i] is out of bounds, return false *)
  val fire_ready : ship -> int -> bool

  (* returns new ship after firing [i]th weapon 
   * if [i] is out of bounds, return ship *)
  val fire : ship -> int -> ship
   ...
   * called beginning of combat *)
  val reset_ship : ship -> ship
